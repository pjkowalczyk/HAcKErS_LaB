...
subtitle: "Discretization: Standard Deviation"
author:
...
#   }
  ...
  chopped <- chop_mean_sd(TMP[, 2], sds = 1)
  
  q <- data.frame(TMP[, 1], TMP[, 2], paste0(names(TMP)[2], '_', chopped), TMP[, 3])
  names(q) <- c('Patient_ID', 'RPKM', 'Tranche', 'EndPt')
  q <- q %>%
    filter(!str_detect(Tranche, '-1 sd, 0 sd')) %>%
    filter(!str_detect(Tranche, '0 sd, 1 sd'))
